...
export default defineConfig(({ mode }) => {
  ...
  const obsidianApiBaseUrl = env.OBSIDIAN_API_BASE_URL ?? 'http://127.0.0.1:27123';
  const obsidianApiToken = env.OBSIDIAN_API_TOKEN ?? '';

  return {
    define: {
      __OBSIDIAN_API_BASE_URL__: JSON.stringify(obsidianApiBaseUrl),
      __OBSIDIAN_API_TOKEN__: JSON.stringify(obsidianApiToken),
    },
    resolve: {
      alias: {
